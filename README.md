...
Backend API for a simple e-commerce order management system, developed
as a practical Senior .NET technical assessment.

The solution focuses on clean architecture, explicit business rules,
...
The solution follows Clean Architecture and is divided into four main
projects:
...
The `Order` entity acts as the aggregate root and controls its
...
The Application layer depends on repository abstractions, while
...
Controllers were chosen instead of Minimal APIs because they provide a
clear HTTP boundary for this assessment while keeping routing,
authorization, and HTTP concerns explicit.

Business logic is not implemented in controllers. Controllers only
receive HTTP requests, delegate use cases to MediatR, and translate
...
For this practical assessment, authentication uses the fixed credentials
...
> For this practical assessment, the JWT signing key is stored in
> application configuration. In a production environment, secrets should
> be provided through environment variables or a secret management
> service.

## Postman Collection

A ready-to-use Postman collection is included with the repository to
make the API easy to evaluate manually.
...
For local execution, configure the Postman environment with:

``` text
baseUrl = http://localhost:5196
```

For Docker execution, use:

``` text
baseUrl = http://localhost:8080
```

After executing the Login request, the returned JWT is automatically
stored in the accessToken collection variable and used by the
authenticated requests.

Example Post-response script for the Login request:
...
Authenticated requests can then use:

``` text
Authorization: Bearer {{accessToken}}
```

This allows the main evaluation flow to be executed directly from
Postman:

``` text
Login → Create Order → List Orders → Get Order By Id → Cancel Order
```
...
Attempting to cancel an order that is not `Pending` results in an HTTP
`409 Conflict`.

## Validation and Error Handling

Application input validation is implemented with FluentValidation and
executed through a MediatR pipeline behavior before the corresponding
handler.
...
-   Domain validation protects business invariants regardless of the
    application entry point.

Errors are translated into consistent ASP.NET Core Problem Details
responses through global exception handling.
...
Pending migrations are automatically applied when the application
starts:
...
`TotalAmount` is intentionally not persisted because it is derived from
the order items by the Domain model.

Read-only queries use `AsNoTracking()`.

Order items are loaded when required because the Domain calculates
`TotalAmount` from the item collection.
...
When running with the current development profile, the API is available
at:
...
EF Core automatically applies pending migrations during application
startup.
...
Domain tests verify business rules independently from Application and
Infrastructure.
...
Repository abstractions are mocked using NSubstitute so these tests
remain isolated from Infrastructure.

### Integration Tests

API integration tests use `WebApplicationFactory` to exercise the real
ASP.NET Core pipeline.

An isolated in-memory SQLite database is used by integration tests,
preventing them from modifying the developer's local database.
...
The .NET SDK image is used only during restore, compilation, and
publishing. The final image contains only the ASP.NET Core runtime and
...
Orders previously created remain available because the named volume is
preserved.
...
Local build artifacts and runtime database files are excluded through
`.dockerignore`.
...
This is particularly important when building a Linux container from a
Windows development environment.

The container performs its own package restore and compilation instead
of reusing environment-specific build artifacts generated on the host.
...
A custom MediatR `LoggingBehavior` records execution time for individual
...
For example, HTTP logging measures the complete request lifecycle, while
the MediatR behavior measures application use-case execution.

Structured logging uses named properties instead of string
concatenation, allowing log platforms to query individual properties
such as request name and execution time.

The application intentionally avoids logging complete command payloads
to reduce the risk of exposing sensitive information.

OpenTelemetry provides distributed tracing for ASP.NET Core requests.

For this practical assessment, traces are exported to the console and
...
This gives the application three complementary observability mechanisms:

``` text
Serilog
    └── Structured application and HTTP logs
...
The Application layer uses CQRS to explicitly separate operations that
modify state from operations that retrieve data.
...
The HTTP layer therefore does not need to know the implementation
...
Consumers can read the items, but modifications are performed through
the aggregate.

Entities use private setters to prevent uncontrolled external state
changes.
...
The Application layer defines persistence abstractions such as
`IOrderRepository`.

Infrastructure implements those abstractions using EF Core.

This keeps application use cases independent from the specific
...
-   OpenTelemetry provides basic distributed tracing with console
    export.
-   Docker uses a multi-stage build.
-   SQLite is persisted through a named Docker volume.
-   Local `bin`, `obj`, and database files are excluded from the Docker
    build context.

## Production Considerations

This project intentionally keeps some concerns simple because it is a
...
-   Production-grade relational database such as PostgreSQL or SQL
    Server.
-   Database migration strategy separated from application startup for
    multi-instance deployments.
-   OpenTelemetry Collector and production observability backend.
-   Additional metrics and distributed tracing instrumentation where
    required.
-   Health checks.
-   CI/CD pipeline.
...
These concerns were intentionally not over-engineered for the scope of
the assessment.
...
The architecture is intentionally explicit so that business rules,
application use cases, infrastructure concerns, and HTTP concerns can
